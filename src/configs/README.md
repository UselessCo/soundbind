...
## Example contributed configs

### `jazz-music.yaml`

A small music-focused config that provides quick-access jazz loops and cues for practice or live performance. Uses `../../assets/sounds/jazz.mp3` and a few built-in sounds as accents. Good for musicians who want simple hotkeys to trigger backing loops and cues.

Quick usage (from project root):

```bash
# Play the jazz config
node index.js ./src/configs/jazz-music.yaml
```
